...
/*-----------------------------------------------------------------------------------
                                  MISC DEFINITIONS
------------------------------------------------------------------------------------*/
#define DEFAULT_LINK_TYPE             PN544_LINK_TYPE_I2C
...
typedef struct Dal_RdWr_st
{
    ...
    uint8_t *             pReadBufferTmp;          /* Read local buffer (to read on I2C in once)*/
    int                   nNbOfBytesToRead;        /* Number of bytes to read */
    int                   nNbOfBytesRead;          /* Number of read bytes */
    int                   nNbOfBytesReadTmp;       /* Number of read bytes (to read on I2C in once)*/
...
NFCSTATUS phDal4Nfc_ConfigRelease(void *pHwRef)
{
   ...
   if (gDalContext.hw_valid == TRUE)
   {
	...
	gLinkFunc.close();
      ...
      // hyogil.kim :: lock issue is found
	pthread_kill(gReadWriteContext.nReadThread, SIGTERM);
...
{
	DAL_PRINT("Read Thread got signal for exit!!\n");
	pthread_exit((void *)NULL);
	DAL_PRINT("exit the read thread!!\n");
}
...
//    pthread_setname_np(pthread_self(), "reader"); // our compiler does not support this
	...
	while (gReadWriteContext.nReadThreadAlive) /* Thread Loop */
	{
		...
		{
			DAL_DEBUG("Break Point 1 gReadWriteContext.nReadThreadAlive [%d]\n" , gReadWriteContext.nReadThreadAlive);
			break;
		}
		...
		if (gReadWriteContext.nReadThreadAlive == 0)
		{
			DAL_DEBUG("Break Point 2 gReadWriteContext.nReadThreadAlive [%d]\n" , gReadWriteContext.nReadThreadAlive);
			break;
		}
	...
	DAL_PRINT("Read Thread pthread_exit!!!!\n");
	pthread_exit((void *)NULL);
...
void phDal4Nfc_DeferredCall(pphDal4Nfc_DeferFuncPointer_t func, void *param)
{
    ...
    phDal4Nfc_msgsnd(nDeferedCallMessageQueueId, (struct msgbuf *)&nDeferedMessageWrapper, sizeof(phLibNfc_Message_t), 0);
#else
    PostMessage((struct msgbuf *)&nDeferedMessageWrapper, sizeof(phLibNfc_Message_t), 0);
#endif
#endif
